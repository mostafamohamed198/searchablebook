...
export default function EntryBookEditForm(){
  let {authTokens} = useContext(AuthContext)
    ...
    React.useEffect (function(){
        fetch('/api/thesearchable/entries/' + params.id)
        .then(res => res.json())
        .then(data => {
            setValues(data)
        })
       
        fetch('/api/thesearchable/booklist')
        .then(res => res.json())
        .then(data => {
            data.map(book => {
    ...
    React.useEffect(function(){
        ...
        fetch(`/api/thesearchable/bookdetail/${bookId}`)
        .then(res => res.json())
        .then(data => {
           setBook(data)
    ...
    const { register, handleSubmit,control, watch, formState: { errors } } = useForm({});
      ...
      const onSubmit = (data) => {
        ...
        fetch('/api/thesearchable/entrybookform/',{
              method: 'POST',
