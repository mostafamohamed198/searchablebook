...
export default function FormEdit(){
  const navigate = useNavigate()
    ...
    React.useEffect (function(){
        fetch(`/api/thesearchable/entrieseditdata/${params.id}/`)
        .then(res => res.json())
        .then(data => {
            setValues(data)
            setLoaded(true)
        })
        fetch('/api/thesearchable/author')
        .then(res => res.json())
        .then(data => {
            // let authorsArray = []
            data.map(author => {
        ...
        fetch('/api/thesearchable/countries')
        .then(res => res.json())
        .then(data => {
            data.map(country=> {
        ...
        fetch('/api/thesearchable/categories')
        .then(res => res.json())
        .then(data => {
            data.map(category=> {
        ...
        fetch('/api/thesearchable/classification')
        .then(res => res.json())
        .then(data => {
            data.map(theclass => {
    ...
    entry:{
      ...
      const onSubmit = (data) => {
        ...
        axios.put(`/api/thesearchable/entrychange/${values.id}/`,{
    ...
    form_data.append('bibiliography', data.bibiliography);
        ...
        axios.put(`/api/thesearchable/entrychange/${values.id}/`, form_data, {
          headers: {
              'Content-Type': 'multipart/form-data',
              'Authorization':'Bearer ' + String(authTokens.access)
          }
