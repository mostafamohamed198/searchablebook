...
export default function BookEditForm(){
   ...
   const [authorOptions, setAuthorOptions] = React.useState([]);
    ...
    React.useEffect (function(){
        ...
        fetch(`/api/thesearchable/bookdetail/${params.id}`)
        .then(res => res.json())
        .then(data => {
            setValues(data)
...
//   let form_data = new FormData();
    const onSubmit= (data) => {            // event.preventDefault()
        if (data.bookClassification != null ){
            ...
            axios.put(`/api/thesearchable/bookchange/${values.id}/`,{
        ...
        axios.put(`/api/thesearchable/bookchange/${values.id}/`, form_data, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                    'Authorization':'Bearer ' + String(authTokens.access)
                }
