...
//     // console.log(hit._snippetResult.body)
...
//         <a style={{textDecoration: 'none'}} href={theID}><h2 style={{color:'#087cc4'}}>{hit.title}</h2></a>
...
export default function SearchResults(){
 const [authentication, setAuthentication] = React.useState(false)
  ...
  return(
    <div>
    {approved? <Results resultvalue={searchvalue} /> : <div>يمكنك عرض محتوي هذه الصفحة من خلال <a href="/login">تسجيل الدخول </a> بحساب مصرح له بعرض المحتوي </div>}
    </div>
  )
}
